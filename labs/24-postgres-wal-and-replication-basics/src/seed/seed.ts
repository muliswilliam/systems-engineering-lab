...
const log = createLogger("lab24:seed");
...
// generator was added to @labs/data-generators, same reasoning as Lab 06's
// `counters`/Lab 11's `documents`/Lab 19's `notifications`.
...
/**
 * Writes ONLY to the primary. This is the point of the lab: the replica is
 * never written to directly, it only ever receives these rows via WAL
 * replay - see src/scenarios/write-to-primary-observe-replica.ts for the
 * script that watches that replay happen row by row.
 */
async function main() {
