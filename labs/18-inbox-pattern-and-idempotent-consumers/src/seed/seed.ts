...
const log = createLogger("lab18:seed");
...
/**
 * Idempotent: clears both tables (`processed_messages` first, since it
 * references `accounts`) and reinserts a fresh, deterministic set of
 * accounts every run - running this twice with the same --seed produces the
 * same logical account balances (SPEC.md 8.1). `processed_messages` starts
 * empty; it is populated only by running one of the consumer scenario
 * scripts or the test suite.
 */
async function main() {
  ...
  log.info({ seed, size, accountCount: inserted.length, totalBalanceCents }, "seed complete");
  await pool.end();
}
