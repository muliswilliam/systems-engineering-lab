...
- [x] 41 - system-design-drills - a synthesis lab, not a new application: no Docker Compose, no database, no `.env.example` (the same "no datastore genuinely needed" allowance Lab 37 used), just eight written system-design drills under `drills/*.md` - the exact eight scenarios named in SPEC.md's own Lab 41 brief (Ticketmaster-style booking, payroll processing, a payment API, a distributed job scheduler, a notification platform, a webhook delivery service, inventory reservation, and a large multi-region SaaS backend) - each walked through SPEC.md's own required eight-step structure (invariants, consistency requirements, storage choice, concurrency mechanism, failure modes, scale estimate, observability, plus a "common wrong answer" section per the parent brief's correct-vs-common-wrong-answer requirement) with every claim tied to a specific earlier lab and its own real captured number rather than restated as generic theory - e.g. the booking drill's rejection of a Redis seat-lock cites Lab 22's own real captured worker-overlap (`worker A writes at 401ms` while `worker B acquires at 261ms` under a 200ms TTL), not an abstract "distributed locks are unsafe" claim, and the multi-region drill's tenant-isolation section cites Lab 39's own real captured leak numbers (`rowsBelongingToOtherTenants: 97500` with RLS off vs. `0` with RLS on, plus the real `FORCE ROW LEVEL SECURITY`-omission owner-bypass gotcha) rather than a generic "use RLS" recommendation. Citations were verified for accuracy against every cited lab's own ROADMAP.md entry during this lab's own development, and span 35 of Labs 01-40 (all of them except 03/04/06/07/24) across all 11 of SPEC.md's phases, confirmed by this lab's own small self-check rather than by hand-count alone. The one runnable piece of this lab, `src/check-citations.ts` (plus `tests/citations.test.ts`, 8 Vitest tests, real captured ~0.2s run, `pnpm typecheck` passing with zero errors), is deliberately NOT a grader for the drills' own open-ended reasoning - it only verifies that every `Lab NN` citation across all 8 drill files resolves to a lab directory that actually exists under `labs/`, that every drill contains all 11 of the required structural headings (`Prompt`/`Model answer`/the 8 SPEC.md-required subsections/`Common wrong answer`/`Interview questions`), and that citations collectively reach at least 9 of SPEC.md's 11 curriculum phases (a breadth floor, not a hand-picked target) - guarding against this lab silently drifting out of sync with the other 40 as they change, without building a scoring engine for a genuinely open-ended design question (see the lab's own README "Architecture" section for why that would be the wrong kind of machinery here). `pnpm lab:test 41` (which just runs `pnpm test` in this directory, the same as Lab 37) and `pnpm check` (a standalone, human-readable citation/phase-coverage report) both verified working. Domain: none - see "Domains by lab" below for why that is a deliberate omission, not an oversight.
...
- Lab 41 (system-design-drills), domain: deliberately none. This is the
  one lab in the curriculum with no schema, no seed data, and no
  Postgres/Redis instance of its own - a synthesis lab over written
  system-design reasoning, not a new hands-on mechanism, so there is no
  domain to name and this note says so explicitly rather than silently
  omitting a "Domain:" line the way every other lab's entry has one. Its
  eight drills each reference other labs' own domains directly instead of
  inventing a ninth (payroll for Drill 02, banking/ledger's own account-
  balance-invariant shape for Drill 03, ticketing for Drill 01, a generic
  SaaS/helpdesk shape reusing Lab 39's own tenants/support_tickets domain
  for Drill 08, and so on) - reusing each cited lab's domain by reference,
  never by importing that lab's actual schema or code, consistent with
  the independent-labs principle.
