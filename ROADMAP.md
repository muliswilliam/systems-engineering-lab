...
- Domains by lab, so far: 01 payroll, 02 payroll, 03 commerce, 04 commerce,
  ...
  one of SPEC.md 8.2's five named domains, 39 a fresh, independent
  SaaS-style helpdesk domain (`tenants` + `support_tickets`) - not one of
  SPEC.md 8.2's five named domains, chosen because an internal admin/debug
  ticket-listing view is a believable, realistic place for exactly the two
  leak classes (a forgotten tenant filter, and a syntactically-fine-but-
  wrong-value one) this lab demonstrates; also the first lab in this
  repository to connect as FOUR distinct, real Postgres roles
