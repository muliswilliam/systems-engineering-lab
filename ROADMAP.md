...
- Domains by lab, so far: 01 payroll, 02 payroll, 03 commerce, 04 commerce,
  ...
  "Architecture"), 23 a fresh, minimal standalone `widgets` table (id/
  public_id/name/value) - not one of SPEC.md 8.2's five named domains,
  same "small standalone table, the lesson is the mechanism, not the data
  model" rationale as Lab 06's `counters`/Lab 11's `documents`/Lab 15's
  `payments`/Lab 19's `notifications`; seeded with Faker called directly in
  `src/seed/seed.ts`, same reasoning as Labs 16/19. This lab is also the
  first to add PgBouncer (`edoburu/pgbouncer`, two instances - one per pool
  mode, see its README "Architecture" for why one instance can't do both)
  as one of CLAUDE.md's explicitly-permitted pieces of additional
  infrastructure beyond Postgres/PGweb.
